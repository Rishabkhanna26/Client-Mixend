...
const ORDER_ITEMS_BY_BUSINESS_TYPE = {
  service: [
...
const buildOrderSeeds = (businessType = "default") => {
  const normalizedType = String(businessType || "").trim().toLowerCase();
  const itemsCatalog =
    ORDER_ITEMS_BY_BUSINESS_TYPE[normalizedType] || ORDER_ITEMS_BY_BUSINESS_TYPE.default;
  const prefix = ORDER_PREFIX[normalizedType] || ORDER_PREFIX.default;
  const statusCycle = [
    "new",
    "confirmed",
    "processing",
...
export const getDummyOrders = (adminId, businessType = "default") => {
  const now = new Date();
  const seeds = buildOrderSeeds(businessType);
