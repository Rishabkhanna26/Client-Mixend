...
export async function GET() {
  try {
    ...
    return Response.json({ success: true, data: settings || { ai_enabled: false, ai_prompt: '', ai_blocklist: '' } });
  } catch (error) {
    if (error.status === 401) {
...
export async function PUT(request) {
  try {
    ...
    const updated = await updateAdminAISettings(user.id, { ai_enabled, ai_prompt, ai_blocklist });
